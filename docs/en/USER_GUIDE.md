...
- local variables with optional initializers: scalars, pointers, named `struct` / `union` objects, and fixed-size arrays with one-dimensional braced initializer lists
...
- declarations and constants: `typedef`, `enum`, named `struct` / `union` definitions with scalar/pointer/array fields, `static_assert` / `_Static_assert`, `bool` / `_Bool`, `true`, `false`, `nullptr`
