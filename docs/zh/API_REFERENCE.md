...
- `Parser`：`parseProgram()`，以及函数定义 / 原型、顶层标量、定长数组、命名 record 和 record 数组全局变量、命名 `struct` / `union` 定义、`alignas` / `_Alignas`、嵌套大括号初始化、嵌套 designator path、compound literal、表达式、比较、加减乘除、`sizeof`、`alignof` / `_Alignof`、`if`、`call`、字面量、变量解析。
