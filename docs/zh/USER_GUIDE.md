...
- 局部变量和可选初始化：标量、指针、命名 `struct` / `union` 对象，以及包含 record 数组在内、带嵌套大括号初始化列表和数组 / 记录 designator path 的定长数组
...
- 声明和常量：`typedef`、`enum`、带标量 / 指针 / 数组 / record 字段的命名 `struct` / `union` 定义、`alignas` / `_Alignas` 对齐说明符、声明和 compound literal 中的 `[2] = 7`、`.field = 7`、`.inner.y = 5`、`[1].field = 7` 这类 designated initializer、`static_assert` / `_Static_assert`、`bool` / `_Bool`、`true`、`false`、`nullptr`
