...
Aurora 是一个 C++17 编译器后端与代码生成练习项目，支持 x86-64 和 macOS arm64 汇编输出。
当前仓库包含 AIR 中间表示、目标抽象层、x86-64 与 AArch64 lowering、x86-64 ELF 可重定位对象写出，以及用于示例工具和测试的 MiniC 前端。
...
- `DEVELOPER_GUIDE.md`：扩展 AIR、目标指令、Pass、MiniC 语法的流程
...
- `X86`：x86-64 目标层具体实现
- `AArch64`：macOS arm64 汇编输出的目标层具体实现
- `CodeGen`：SelectionDAG 框架、指令选择、寄存器分配、Pass 管线
- `MC`：汇编打印、x86-64 对象编码、x86-64 ELF relocatable writer
