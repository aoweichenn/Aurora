...
- `include/minic/parse/` and `src/parse/`: parser facade, token cursor, type/declaration parsing, statements, initializers, compound literals, and expression precedence parsing.
- `include/minic/codegen/` and `src/codegen/`: codegen facade, AIR type conversion, function emission, declarations, statements, expressions, compound literal lowering, and analysis helpers.
