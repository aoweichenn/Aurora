...
- `include/minic/parse/` and `src/parse/`: parser facade, token cursor, type/declaration parsing, alignment specifiers, statements, nested initializers, designator paths, compound literals, and expression precedence parsing.
- `include/minic/codegen/` and `src/codegen/`: codegen facade, AIR type conversion, function emission, declarations, statements, expressions, nested initializer, designator-path, and compound literal lowering, and analysis helpers.
