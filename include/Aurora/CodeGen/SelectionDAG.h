...
public:
    ...
    void setNodeId(unsigned id) noexcept { nodeId_ = id; }
    ...
    void setSelected(bool s) noexcept { selected_ = s; }
    uint16_t getMachineOpcode() const noexcept { return machOpcode_; }
    void setMachineOpcode(uint16_t op) noexcept { machOpcode_ = op; }

    unsigned getVReg() const noexcept { return vreg_; }
    void setVReg(unsigned v) noexcept { vreg_ = v; }
...
public:
    ...
    SDValue(SDNode* node, unsigned resNo = 0) : node_(node), resNo_(resNo) {}
